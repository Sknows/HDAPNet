...
class SDI(nn.Module):
    def __init__(self, channels):
        ...
        self.weights = nn.Parameter(torch.ones(len(channels), dtype=torch.float32), requires_grad=True)
        self.relu = nn.ReLU()
        self.epsilon = 1e-4

    def forward(self, xs):
        # Step 1: 自底向上的融合（Bottom-up Fusion）
        # 下采样操作传递信息
        bottom_up_features = [xs[0]]
        for i in range(1, len(xs)):
            x = F.adaptive_avg_pool2d(xs[i], bottom_up_features[-1].shape[-2:])
            bottom_up_features.append(bottom_up_features[-1] + x)  # 将低分辨率特征与高分辨率特征融合

        # Step 2: 自顶向下的融合（Top-down Fusion）
        top_down_features = [bottom_up_features[-1]]
        for i in range(len(xs) - 2, -1, -1):
            x = F.interpolate(top_down_features[-1], size=bottom_up_features[i].shape[-2:], mode='bilinear', align_corners=True)
            top_down_features.append(x + bottom_up_features[i])  # 将上层的低分辨率特征与下层的高分辨率特征融合

        top_down_features = top_down_features[::-1]  # 反转特征列表，使之按从高分辨率到低分辨率的顺序排列

        # Step 3: 加权融合所有特征
        fused_features = torch.zeros_like(xs[0])
        weights = self.relu(self.weights.clone())
        weight_sum = torch.sum(weights) + self.epsilon

        for i, x in enumerate(top_down_features):
            x = self.convs[i](x)  # 每个特征图通过卷积处理
            fused_features += (weights[i] * x) / weight_sum  # 加权融合
...
class PartiallyTransformerBlock(nn.Module):
    def __init__(self, c, tcr, shortcut=True) -> None:
        ...
        self.c_b = Bottleneck_AKConv(self.c_ch, self.c_ch, shortcut=shortcut)
