...
from ..modules import Conv, DFL, C2f, RepConv, Proto, Detect, Segment, Pose, OBB, DSConv, v10Detect,DWConv
...
class Detect_DyHead(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, block_num=4, ch=()):  # detection layer
        super().__init__()
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(DWConv(hidc, hidc, 3), Conv(hidc, c3, 1)),  # 第一层：从 hidc 到 c3
                nn.Sequential(DWConv(c3, c3, 3), Conv(c3, c3, 1)),  # 第二层：在 c3 通道上再处理
                nn.Conv2d(c3, self.nc, 1),  # 最后一层：输出类别通道
            )
            for _ in ch
        )
